...
policy_df (tùy chọn): DataFrame chứa chính sách ca làm việc với các cột:
...
Trả về:
Đối tượng datetime nếu parse thành công, None nếu thất bại.
...
Tại sao giá trị NaN xuất hiện
Giá trị NaN có thể xuất hiện trong kết quả của code do các nguyên nhân sau:

Dữ liệu timestamp không hợp lệ:

Hàm parse_timestamp trả về None nếu chuỗi thời gian không đúng định dạng YYYYMMDDHHMMSS (ví dụ: chuỗi rỗng, định dạng sai như "2025-08-04", hoặc giá trị không phải chuỗi). Điều này dẫn đến NaN trong cột datetime của df, và các phép tính tiếp theo như fci.date() hoặc (lco - fci).total_seconds() có thể tạo ra NaN trong các cột như Ngày chấm công hoặc Thời lượng (h).


Xử lý fci và lco không an toàn:

Trong process_attendance, nếu fci hoặc lco là NaN (do lỗi từ parse_timestamp), phép tính duration = (lco - fci).total_seconds() / 3600 sẽ trả về NaN. Mặc dù code có kiểm tra pd.isna(fci) và pd.isna(lco), việc định dạng chuỗi như fci.strftime hoặc lco.strftime mà không kiểm tra đầy đủ có thể gây lỗi gián tiếp, dẫn đến NaN trong các cột như FirstCheckIn, LastCheckOut, Giờ vào, hoặc Giờ ra.


Lỗi trong apply_policy_adjustments:

Hàm parse_shift_time trả về None nếu thời gian trong policy_df (như start_day, end_day) không hợp lệ (ví dụ: chuỗi rỗng, "25:00:00"). Điều này dẫn đến shift_start hoặc shift_end là None, làm cho duration_2 = (shift_end - shift_start).total_seconds() / 3600 trả về NaN.
Nếu FirstCheckIn hoặc LastCheckOut không thể parse đúng định dạng %d/%m - %H:%M:%S, duration_1 cũng có thể là NaN.


Dữ liệu đầu vào không nhất quán:

Nếu df chứa các giá trị timestamp không hợp lệ, thiếu cột, hoặc policy_df có các giá trị thời gian sai định dạng (như "abc" thay vì "08:00:00"), các phép tính thời lượng và gán giá trị sẽ dẫn đến NaN.


Log đơn lẻ không được xử lý đầy đủ:

Trong handle_single_logs, các bản ghi cho log đơn lẻ gán Thời lượng (h) = 0, nhưng nếu row['datetime'] không hợp lệ, việc định dạng chuỗi có thể gây ra lỗi gián tiếp, dẫn đến NaN trong các cột khác.



Giải pháp:

Hàm remove_nan_rows được gọi cuối cùng để loại bỏ các dòng có NaN trong các cột quan trọng (Thời lượng (h), Giờ vào, Giờ ra, FirstCheckIn, LastCheckOut). Tuy nhiên, để ngăn NaN từ đầu, cần:
Kiểm tra và làm sạch dữ liệu đầu vào (df và policy_df) trước khi xử lý.
Sử dụng errors='coerce' trong pd.to_datetime để xử lý các giá trị thời gian không hợp lệ.
Thêm debug để ghi lại các giá trị timestamp hoặc thời gian trong policy_df gây lỗi.



Debug:

Kiểm tra NaN trong df sau khi parse:print(df[df['datetime'].isna()])


Kiểm tra policy_df:print(policy_df[['start_day', 'start_night', 'end_day', 'end_night']].isna().sum())
...
Xử lý NaN:
Các dòng có NaN trong datetime được loại bỏ ngay từ đầu.
Hàm remove_nan_rows loại bỏ các dòng có NaN trong các cột quan trọng.


Dữ liệu đầu vào:
Đảm bảo cột timestamp có định dạng đúng (YYYYMMDDHHMMSS).
Nếu policy_df được cung cấp, các cột thời gian phải là HH:MM:SS hoặc HH:MM.


Hiệu suất:
Code xử lý từng nhóm nhân viên, phù hợp với dữ liệu lớn, nhưng cần đảm bảo dữ liệu đầu vào sạch để tránh lỗi.


Debug:
Kiểm tra NaN bằng: print(df_result.isna().sum()).
Kiểm tra dữ liệu đầu vào: print(df[['timestamp', 'id', 'key']].head()).
